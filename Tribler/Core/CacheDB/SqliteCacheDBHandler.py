    ...
    def getPeer(self, permid, keys=None):
        if keys is not None:
            ...
            value_name = ('permid', 'name', 'ip', 'port', 'similarity', 'friend',
		      ...
		      'gc_member', 'gc_last_seen', 'gc_last_recieved')
...
class SuperPeerDBHandler(BasicDBHandler):
    ...
    def loadSuperPeers(self, config, refresh=False):
        ...
        filename = os.path.join(config['install_dir'], config['gc_superpeer_file'])
        superpeer_list = self.readSuperPeerList(filename)
        for superpeer in superpeer_list:
            superpeer['gc_member'] = 1
        self.insertSuperPeers(superpeer_list, refresh)
...
class VoteCastDBHandler(BasicDBHandler):
    ...
    def getAllPosNegVotes(self, mod_ids = None):
        if mod_ids:
            mod_ids = "AND mod_id IN ('" + "' ,'".join(mod_ids) + "') "
        else:
            mod_ids = ''

        votes = {}
        sql = 'select mod_id, count(*) from VoteCast where vote == 2 '+mod_ids+'group by mod_id'
        records = self._db.fetchall(sql)
        for mod_id, pos_votes in records:
            votes[mod_id] = [pos_votes, 0]

        sql = 'select mod_id, count(*) from VoteCast where vote == -1 '+mod_ids+'group by mod_id'
        records = self._db.fetchall(sql)
        for mod_id, neg_votes in records:
            if mod_id not in votes:
                votes[mod_id] = [0, neg_votes]
            else:
                votes[mod_id][1] = neg_votes
...
class ChannelCastDBHandler(BasicDBHandler):
    ...
    def getTorrentFromPublisherId(self, publisher_id, infohash):
        ...
        where = "T.infohash == C.infohash AND publisher_id=='%s' AND t.infohash=='%s' AND name<>''"% (publisher_id, bin2str(infohash))
    ...
    def _getChannels(self, sql, args = None, maxvotes = sys.maxint, minvotes = 0, cmpF = None, allChannels = False):
        ...
        for result in results:
            if result[0] not in votedict:
                votedict[result[0]] = self.votecast_db.getPosNegVotes(result[0])

            nr_favorites, nr_spam = votedict[result[0]]
    ...
    def getSubscribersCount(self,permid):
        """returns the number of subscribers in integer format"""
        sql = "select count(*) from VoteCast where mod_id==? and vote=2"
        numrecords = self._db.fetchone(sql, (bin2str(permid),))
        return numrecords
...
class GameCastDBHandler(BasicDBHandler):
    ...
    def getInstance(*args, **kw):
        # Singleton pattern with double-checking
        if GameCastDBHandler.__single is None:
            GameCastDBHandler.lock.acquire()
            try:
                if GameCastDBHandler.__single is None:
                    GameCastDBHandler(*args, **kw)
            finally:
                GameCastDBHandler.lock.release()
        return GameCastDBHandler.__single

    getInstance = staticmethod(getInstance)

    def __init__(self):
        if GameCastDBHandler.__single is not None:
            raise RuntimeError, "GameCastDBHandler is singleton"
        GameCastDBHandler.__single = self
        db = SQLiteCacheDB.getInstance()
        BasicDBHandler.__init__(self, db, 'Game')
        self.peer_db = PeerDBHandler.getInstance()

    def getHighScores(self, num, gamename):
        # This method is only used by the GUI
        sql = """SELECT permid, rating
                 FROM GameRatings
                 WHERE gamename = '"""+gamename+"""'
                 ORDER BY rating desc"""
        ratings = self._db.fetchall(sql)
        if ratings is None:
            ratings = []
        else:
            ratings = ratings[:num]
    ...
    def getMyInvites(self):
        # This method is only used by the GUI
        sql = """SELECT invite_id, owner_id, target_id, game_id, min_rating, max_rating, time, inc, gamename, colour, status, creation_time
                 FROM GameInvite
                 WHERE owner_id = 0"""
        my_invites = self._db.fetchall(sql)
        result = []
        if not my_invites:
            return result
        keys = ('invite_id', 'owner_id', 'target_id', 'game_id', 'min_rating', 'max_rating', 'time', 'inc', 'gamename', 'colour', 'status', 'creation_time')
        for i in my_invites:
            invite = dict(zip(keys,i))
            result.append(invite)
        return result

    def getCurrentInvites(self, myrating, gamename):
        # This method is only used by the GUI
        sql = """SELECT invite_id, owner_id, target_id, game_id, min_rating, max_rating, time, inc, gamename, colour, status, creation_time
                 FROM GameInvite
                 WHERE (target_id = 0 or (target_id = -1 and min_rating <= """+str(myrating)+""" and max_rating >= """+str(myrating)+"""))
                       and owner_id != 0 and gamename = '"""+gamename+"""'"""
        cur_invites = self._db.fetchall(sql)
        result = []
        if not cur_invites:
            return result
        keys = ('invite_id', 'owner_id', 'target_id', 'game_id', 'min_rating', 'max_rating', 'time', 'inc', 'gamename', 'colour', 'status', 'creation_time')
        for i in cur_invites:
            invite = dict(zip(keys,i))
            result.append(invite)
        return result

    def getCurrentGames(self, permid, finished = False):
        # This method is only used by the GUI
        sql = """SELECT game_id, owner_id, winner_permid, moves, players, gamename, time, inc, is_finished, finished_time, lastmove_time, creation_time
                 FROM Game
                 WHERE players LIKE '%"""+bin2str(permid)
        if finished:
	    sql += """%' and is_finished > 0"""
        else:
            sql += """%' and is_finished = 0"""
        cur_games = self._db.fetchall(sql)
        result = []
        if not cur_games:
            return result
        keys = ('game_id', 'owner_id', 'winner_permid', 'moves', 'players', 'gamename' , 'time', 'inc', 'is_finished', 'finished_time', 'lastmove_time', 'creation_time')
        for g in cur_games:
            game = dict(zip(keys,g))
            game['winner_permid'] = str2bin(game['winner_permid'])
            game['moves'] = bdecode(game['moves'])
            game['players'] = dict((str2bin(key), value) for (key, value) in bdecode(game['players']).items())
            result.append(game)
        return result

    def getGamesSince(self, permid, timestamp):
        # This method is only used by the GUI
        sql = """SELECT game_id, owner_id, winner_permid, moves, players, gamename, time, inc, is_finished, finished_time, lastmove_time, creation_time
                 FROM Game
                 WHERE players LIKE '%"""+bin2str(permid)+"""%' and (is_finished = 0 or finished_time > """+str(int(timestamp))+""")"""
        cur_games = self._db.fetchall(sql)
        result = []
        if not cur_games:
            return result
    ...
    def getNextGameID(self, owner_id = 0):
        game_id = self._db.fetchone("SELECT MAX(game_id) FROM Game WHERE owner_id = "+str(owner_id))
        if game_id == None: game_id = 1
        else: game_id += 1
        return game_id

    def addGame(self, value, commit=True):
        self._db.insert(self.table_name, commit=commit, **value)
        self.notifier.notify(NTFY_GAMECAST, NTFY_INSERT, 'Game', (value['game_id'], value['owner_id']))

    def updateGame(self, value, commit=True):
        value = copy(value)
        game_id = value.pop('game_id')
        owner_id = value.pop('owner_id')
        where = "game_id=" + str(game_id) + " and owner_id=" + str(owner_id)
        self._db.update(self.table_name, where, commit=commit, **value)
        self.notifier.notify(NTFY_GAMECAST, NTFY_UPDATE, 'Game', (game_id, owner_id))

    def deleteGame(self, game_id, owner_id, commit=True):
        self._db.delete('Game', commit=commit, game_id=game_id, owner_id=owner_id)
        self.notifier.notify(NTFY_GAMECAST, NTFY_DELETE, 'Game', (game_id, owner_id))

    def getGames(self, **kwargs):
        keys = ('game_id', 'owner_id', 'winner_permid', 'moves', 'players', 'gamename' , 'time', 'inc', 'is_finished', 'finished_time', 'lastmove_time', 'creation_time')
        games = self._db.getAll(self.table_name, keys, **kwargs)
        if games is None:
            games = []
    ...
    def getMyRecentGames(self, num):
        sql = """SELECT game_id, owner_id, winner_permid, moves, players, time, inc, gamename, finished_time, lastmove_time, is_finished
                 FROM Game
                 WHERE is_finished > 0 and owner_id = 0
                 ORDER BY creation_time desc"""
        recent_games = self._db.fetchall(sql)
        if recent_games is None:
            recent_games = []
        recent_games = recent_games[:num]

        keys = ('game_id', 'owner_id', 'winner_permid', 'moves', 'players', 'time', 'inc', 'gamename', 'finished_time', 'lastmove_time', 'is_finished')
        result = []
    ...
    def getRating(self, permid, gamename):
        value = {'permid':bin2str(permid), 'gamename':gamename}
        keys = ('rating', 'rd')
        rating = self._db.getAll('GameRatings', keys, **value)
        if rating is None or len(rating) == 0:
            return {}
        rating = dict(zip(keys,rating[0]))
        return rating

    def setRating(self, permid, gamename, rating_tuple, commit=True):
        value = {}
        value['rating'] = rating_tuple[0]
        value['rd'] = rating_tuple[1]
        if not self.getRating(permid, gamename):
            value['permid'] = bin2str(permid)
            value['gamename'] = gamename
            self._db.insert('GameRatings', commit=commit, **value)
            self.notifier.notify(NTFY_GAMECAST, NTFY_INSERT, 'GameRatings', (value['permid'], value['gamename']))
        else:
            where = "permid='" + bin2str(permid) + "' and gamename='" + gamename +"'"
            self._db.update('GameRatings', where, commit=commit, **value)
            self.notifier.notify(NTFY_GAMECAST, NTFY_UPDATE, 'GameRatings', (permid, gamename))

    def getNextMessageID(self, owner_id = 0):
        message_id = self._db.fetchone("SELECT MAX(message_id) FROM GameMessage WHERE owner_id = "+str(owner_id))
        if message_id == None: message_id = 1
        else: message_id += 1
        return message_id

    def addMessage(self, value, commit=True):
        self._db.insert('GameMessage', commit=commit, **value)
        self.notifier.notify(NTFY_GAMECAST, NTFY_INSERT, 'GameMessage', (value['message_id'], value['owner_id']))

    def updateMessage(self, value, commit=True):
        value = copy(value)
        message_id = value.pop('message_id')
        owner_id = value.pop('owner_id')
        where = "message_id=" + str(message_id) + " and owner_id=" + str(owner_id)
        self._db.update('GameMessage', where, commit=commit, **value)
        self.notifier.notify(NTFY_GAMECAST, NTFY_UPDATE, 'GameMessage', (message_id, owner_id))

    def deleteMessage(self, message_id, owner_id, commit=True):
        self._db.delete('GameMessage', commit=commit, message_id=message_id, owner_id=owner_id)
        self.notifier.notify(NTFY_GAMECAST, NTFY_DELETE, 'GameMessage', (message_id, owner_id))

    def getMessages(self, **kwargs):
        keys = ('message_id', 'owner_id', 'game_id', 'game_owner_id', 'content', 'creation_time')
        messages = self._db.getAll('GameMessage', keys, **kwargs)
        if messages is None:
            messages = []

        result = []
        for m in messages:
            message = dict(zip(keys,m))
            result.append(message)
        return result

    def getNextInviteID(self, owner_id = 0):
        invite_id = self._db.fetchone("SELECT MAX(invite_id) FROM GameInvite WHERE owner_id = "+str(owner_id))
        if invite_id == None: invite_id = 1
        else: invite_id += 1
        return invite_id

    def addInvite(self, value, commit=True):
        self._db.insert('GameInvite', commit=commit, **value)
        self.notifier.notify(NTFY_GAMECAST, NTFY_INSERT, 'GameInvite')

    def updateInvite(self, value, commit=True):
        value = copy(value)
        invite_id = value.pop('invite_id')
        owner_id = value.pop('owner_id')
        where = "invite_id=" + str(invite_id) + " and owner_id=" + str(owner_id)
        self._db.update('GameInvite', where, commit=commit, **value)
        self.notifier.notify(NTFY_GAMECAST, NTFY_UPDATE, 'GameInvite')

    def deleteInvite(self, invite_id, owner_id, commit=True):
        self._db.delete('GameInvite', commit=commit, invite_id=invite_id, owner_id=owner_id)
        self.notifier.notify(NTFY_GAMECAST, NTFY_DELETE, 'GameInvite', (invite_id, owner_id))

    def getInvites(self, **kwargs):
        keys = ('invite_id', 'owner_id', 'target_id', 'game_id', 'min_rating', 'max_rating', 'time', 'inc', 'gamename', 'colour', 'status', 'creation_time')
        invites = self._db.getAll('GameInvite', keys, **kwargs)
        if invites is None:
            invites = []
    ...
    def getResults(self, gamename):
        sql = """SELECT finished_time, players, winner_permid
                 FROM Game
                 WHERE is_finished > 0 and gamename = '"""+gamename+"""'
        ...
        for result in results:
            lmtime = result[0]
            players = dict((str2bin(key), value) for (key, value) in bdecode(result[1]).items())
            winner = str2bin(result[2])
            retval.append((lmtime, players, winner))
        return retval

    def getInteractionCount(self, permid):
        sql = """SELECT players
                 FROM Game
                 WHERE players LIKE '%"""+bin2str(permid)+"""%' and is_finished > 0"""
        my_games = self._db.fetchall(sql)
        result = {}
        if not my_games:
            return result
        for game in my_games:
            players = bdecode(game[0])
            for player in players:
                player = str2bin(player)
                if player == permid:
                    continue
                if player in result:
                    result[player] += 1
